...
test("server-renders the complete AI Video Agent landing page", async () => {
  ...
  assert.match(html, /AI Video Agent Questions and Answers/);
  assert.match(html, /What is an AI video agent\?/);
...
test("covers the verified keyword cluster through useful page modules", async () => {
  ...
  assert.match(html, /End-to-end AI video generator/);
  assert.match(html, /AI agent for video creation/);
  assert.match(html, /Script to video AI agent/);
  assert.match(html, /AI video workflow automation/);
  assert.match(html, /AI Video Agent FAQ/);
...
test("keeps product claims inside the verified fact boundary", async () => {
  ...
  assert.match(html, /plan scenes, create or gather visual assets, assemble an edit/i);
  assert.match(html, /end-to-end AI video generation/i);
  assert.match(html, /Access and credit requirements depend on your Wizstar account/i);
...
test("routes every user-facing link to the Wizstar home entry point", async () => {
  const response = await render();
  const html = await response.text();
  const hrefs = [...html.matchAll(/<a\b[^>]*\bhref="([^"]+)"/gi)].map((match) => match[1]);
  assert.ok(hrefs.length > 0, "expected rendered anchors");
  assert.ok(hrefs.every((href) => href === "https://wizstar.com/home"), `unexpected link targets: ${hrefs.join(", ")}`);
});
